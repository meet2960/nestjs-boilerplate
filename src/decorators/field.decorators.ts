...
import { supportedLanguageCount } from '@/common/constants/language-code';
import type { Constructor } from '@/common/types/common-types';
...
export function StringField(
  ...
  if (options.nullable) {
    ...
    decorators.push(NotEquals(null, { each: options.each }));
    decorators.push(Transform(({ value }) => trim(value) ?? value));
  }
...
export function DateField(
  ...
  if (options.swagger !== false) {
    decorators.push(
      ApiProperty({
        type: Date,
        default: options.default ?? new Date().toISOString(),
        ...(options as ApiPropertyOptions),
      }),
    );
  }
