...
import type { ICreateLoginHistoryRecord } from './types/types';
...
export class AuthService {
  ...
  async loginUser(data: UserLoginDto, req: Request, _res: Response) {
    try {
      ...
      // const isPasswordValid = user.password === decryptedPassword;
  ...
  async logout(_data: Record<string, unknown>, req: Request, _res: Response) {
  ...
  async getCurrentUser(_user: IUserSession) {
    try {
      // const findUser = await this.userService.findOne(user.user_id);
      // if (!findUser.data) {
      //   return this.responseType.errorResponse({
  ...
  async changePassword(_data: ChangePasswordDto, _user: IUserSession) {
    try {
